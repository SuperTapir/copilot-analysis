...
});,var _LineWithValueAndCost,
  LineWithValueAndCost,
  init_lineWithValueAndCost = __esmMin(() => {
    "use strict";

    init_tokenization();
    _LineWithValueAndCost = class _LineWithValueAndCost {
...
`)) {
    ...
    LineWithValueAndCost = _LineWithValueAndCost;
...
`);
  ...
  init_elidableText = __esmMin(() => {
    ...
    _ElidableText = class _ElidableText {
      constructor(chunks) {
        this.lines = [];
        let lines = [];
...
`)) {
    ...
    ElidableText = _ElidableText;
    __name(makePrompt, "makePrompt");
  });,function Diff() {},function buildValues(diff, lastComponent, newString, oldString, useLongestToken) {
  ...
  return componentLen > 1 && typeof finalComponent.value == "string" && (finalComponent.added || finalComponent.removed) && diff.equals("", finalComponent.value) && (components[componentLen - 2].value += finalComponent.value, components.pop()), components;
...
`);
  ...
  init_lib = __esmMin(() => {
    ...
    Diff.prototype = {
      diff: __name(function (oldString, newString) {
        ...
        var maxExecutionTime = (_options$timeout = options.timeout) !== null && _options$timeout !== void 0 ? _options$timeout : 1 / 0,
          ...
          newPos = this.extractCommon(bestPath[0], newString, oldString, 0);
        ...
        function execEditLength() {
          for (var diagonalPath = Math.max(minDiagonalToConsider, -editLength); diagonalPath <= Math.min(maxDiagonalToConsider, editLength); diagonalPath += 2) {
            ...
            if (!canRemove || canAdd && removePath.oldPos + 1 < addPath.oldPos ? basePath = self.addToPath(addPath, !0, void 0, 0) : basePath = self.addToPath(removePath, void 0, !0, 1), newPos = self.extractCommon(basePath, newString, oldString, diagonalPath), basePath.oldPos + 1 >= oldLen && newPos + 1 >= newLen) return done(buildValues(self, basePath.lastComponent, newString, oldString, self.useLongestToken));
      ...
      addToPath: __name(function (path, added, removed, oldPosInc) {
      ...
      extractCommon: __name(function (basePath, newString, oldString, diagonalPath) {
        for (var newLen = newString.length, oldLen = oldString.length, oldPos = basePath.oldPos, newPos = oldPos - diagonalPath, commonCount = 0; newPos + 1 < newLen && oldPos + 1 < oldLen && this.equals(newString[newPos + 1], oldString[oldPos + 1]);) newPos++, oldPos++, commonCount++;
        return commonCount && (basePath.lastComponent = {
          count: commonCount,
    ...
    lineDiff.tokenize = function (value) {
      this.options.stripTrailingCr && (value = value.replace(/\r\n/g, `
`));
      var retLines = [],
