...
},__name(hasComment, "hasComment");,var _PromptFeatures = class _PromptFeatures {
...
`),
...
};,__name(_PromptFeatures, "PromptFeatures");,var PromptFeatures = _PromptFeatures,
  ...
  };,__name(_MultilineModelFeatures, "MultilineModelFeatures");,var MultilineModelFeatures = _MultilineModelFeatures;,function constructMultilineFeatures(prompt, language) {
...
`]), requestContext.multiline && requestContext.multiLogitBias && (postOptions.logit_bias = {
  ...
  return genericGetCompletionsFromNetwork(ctx, requestContext, baseTelemetryData, cancellationToken, finishedCb, "completions", async (numGhostCompletions, requestStart, processingTime, choicesStream) => {
    ...
    let cacheDone = Promise.all(remainingPromise).then(async results => {
      ctx.get(Features).fastCancellation(baseTelemetryData) && choicesIterator.next(), ghostTextLogger.debug(ctx, `Awaited remaining results, number of results: ${results.length}`);
      let apiChoices = [];
      for (let innerChoice of results) {
        let redactedChoice = innerChoice.value;
        if (redactedChoice !== void 0 && (ghostTextLogger.debug(ctx, `GhostText later completion: ${JSON.stringify(redactedChoice.completionText)}`), redactedChoice.completionText.trimEnd())) {
...
`)[0]), ghostChoice;
},__name(makeGhostAPIChoice, "makeGhostAPIChoice");,async function getNumGhostCompletions(ctx, requestContext, telemetryData) {
  let override = ctx.get(Features).overrideNumGhostCompletions(telemetryData);
  return override ? requestContext.isCycling ? Math.max(0, 3 - override) : override : shouldDoParsingTrimming(requestContext.blockMode) && requestContext.multiline ? 3 : requestContext.isCycling ? 2 : 1;
...
},__name(getGhostTextStrategy, "getGhostTextStrategy");,var ghostTextDebouncer = new Debouncer();,async function getGhostText(ctx, document, position, isCycling, preIssuedTelemetryData, cancellationToken, ifInserted) {
  var _a, _b;
  ...
  if (ghostTextStrategy.isCyclingRequest && ((_a = choices == null ? void 0 : choices[0].length) != null ? _a : 0) > 1 || !ghostTextStrategy.isCyclingRequest && choices !== void 0) ghostTextLogger.debug(ctx, "Found inline suggestions locally");else {
...
},__name(exploreMultilineRandom, "exploreMultilineRandom");,var _ForceMultiLine = class _ForceMultiLine {
  constructor(requestMultilineOverride = !1) {
    this.requestMultilineOverride = requestMultilineOverride;
  }
};,__name(_ForceMultiLine, "ForceMultiLine"), _ForceMultiLine.default = new _ForceMultiLine();,var ForceMultiLine = _ForceMultiLine;,async function requestMultilineExperiment(requestMultilineExploration, requestMultiModel, requestMultiModelThreshold, document, prompt) {
  ...
  if (document.lineCount >= 8e3) telemetry(ctx, "ghostText.longFileMultilineSkip", TelemetryData.createAndMarkAsIssued({
    ...
    let requestMultiline = !1;
    return !inlineSuggestion && (0, oP.isSupportedLanguageId)(document.languageId) ? requestMultiline = await isEmptyBlockStart(document, position) : inlineSuggestion && (0, oP.isSupportedLanguageId)(document.languageId) && (requestMultiline = (await isEmptyBlockStart(document, position)) || (await isEmptyBlockStart(document, document.lineAt(position).range.end))), requestMultiline || (requestMultiline = await requestMultilineExperiment(requestMultilineExploration, requestMultiModel, requestMultiModelThreshold, document, prompt)), requestMultiline;
  }
  return !1;
...
`).length
...
},__name(telemetryWithAddData, "telemetryWithAddData");,function telemetryIssued(ctx, document, requestContext, position, prompt, baseTelemetryData, computeContextualFilterScore, contextualFilterEnableTree) {
  ...
  let typeFileHashCode = Array.from(prompt.neighborSource.entries()).map(typeFiles => [typeFiles[0], typeFiles[1].map(f => (0, tX.SHA256)(f).toString())]);
