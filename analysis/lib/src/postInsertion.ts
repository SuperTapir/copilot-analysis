var postInsertionLogger = new Logger(1, "postInsertion"),
  captureTimeouts = [{
    seconds: 15,
    captureCode: !1,
    captureRejection: !1
  ...
  postInsertConfiguration = {
    ...
    captureCode: !1,
    captureRejection: !1
  };,async function captureCode(ctx, fileURI, completionTelemetry, offset, suffixOffset) {
  ...
  captureTimeouts.filter(t => t.captureRejection).map(t => {
    positionTracker.push(async () => {
      ...
      let {
        ...
        } = await captureCode(ctx, fileURI, completionTelemetryData, positionTracker.offset + 1, suffixTracker.offset),
        promptTelemetry;
...
},__name(postRejectionTasks, "postRejectionTasks");,async function postInsertionTasks(ctx, insertionCategory, completionText, insertionOffset, fileURI, telemetryData, suggestionStatus, completionId, start) {
  ...
  postInsertionLogger.debug(ctx, `${insertionCategory}.accepted choiceIndex: ${telemetryDataWithStatus.properties.choiceIndex}`), telemetryAccepted(ctx, insertionCategory, telemetryDataWithStatus), completionText = computeCompletionText(completionText, suggestionStatus);
  let trimmedCompletion = completionText.trim(),
    ...
    stillInCodeCheck = __name(async timeout => {
      await checkStillInCode(ctx, insertionCategory, trimmedCompletion, insertionOffset, fileURI, timeout, telemetryDataWithStatus, tracker, suffixTracker);
    }, "stillInCodeCheck");
  ...
  } else captureTimeouts.map(timeout => tracker.push(() => stillInCodeCheck(timeout), timeout.seconds * 1e3));
  ...
  if (document) {
    ...
    if (telemetry(ctx, insertionCategory + ".stillInCode", customTelemetryData), timeout.captureCode) {
      let {
          ...
          capturedCode: capturedCode,
          terminationOffset: terminationOffset
        } = await captureCode(ctx, fileURI, customTelemetryData, tracker.offset, suffixTracker.offset),
        promptTelemetry;
      prompt.isFimEnabled ? promptTelemetry = {
        hypotheticalPromptPrefixJson: JSON.stringify(prompt.prefix),
        hypotheticalPromptSuffixJson: JSON.stringify(prompt.suffix)
