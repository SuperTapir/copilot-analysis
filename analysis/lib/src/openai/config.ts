var OPENAI_PROXY_HOST = "https://copilot-proxy.githubusercontent.com",
  V1_ENGINES_COPILOT_CODEX = "/v1/engines/copilot-codex";,function _getOverrideProxyURL(ctx) {
...
},__name(getProxyURLWithPath, "getProxyURLWithPath");,async function _getEnginePath(ctx, telemetryData) {
  let engineOverride = getConfig(ctx, ConfigKey.DebugOverrideEngine);
  if (engineOverride) return `/v1/engines/${engineOverride}`;
  let customEngine = ctx.get(Features).customEngine(telemetryData);
  return customEngine !== "" ? `/v1/engines/${customEngine}` : V1_ENGINES_COPILOT_CODEX;
},__name(_getEnginePath, "_getEnginePath");,async function getEngineURL(ctx, telemetryData) {
  return getProxyURLWithPath(ctx, await _getEnginePath(ctx, telemetryData));
