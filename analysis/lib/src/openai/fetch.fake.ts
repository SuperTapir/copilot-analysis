var import_copilot_promptlib = Dr(Dm());,function fakeAPIChoice(headerRequestId, choiceIndex, completionText, telemetryData = TelemetryData.createAndMarkAsIssued()) {
  let tokenizer = (0, Bpe.getTokenizer)();
  return {
...
},__name(fakeResponse, "fakeResponse");,var _SyntheticCompletions = class _SyntheticCompletions extends OpenAIFetcher {
  ...
  async fetchAndStreamCompletions(ctx, params, baseTelemetryData, finishedCb, cancel, teletryProperties, allowEmptyChoices) {
    if (ctx.get(CopilotTokenManager).getCopilotToken(ctx), cancel != null && cancel.isCancellationRequested) return {
      type: "canceled",
...
};,__name(_SyntheticCompletions, "SyntheticCompletions");,var SyntheticCompletions = _SyntheticCompletions;