var import_value = Dr(NT());,var assertShape = __name((schema, payload) => {
  if (TT.Value.Check(schema, payload)) return payload;
  let error = `Typebox schema validation failed:
${[...TT.Value.Errors(schema, payload)].map(i => `${i.path} ${i.message}`).join(`
`)}`;
...
},__name(fileBlockedEvaluationResult, "fileBlockedEvaluationResult");,var SourceSchema = LA.Type.Object({
    name: LA.Type.String(),
    type: LA.Type.Union([LA.Type.Literal("Organization"), LA.Type.Literal("Repository")])
  }),
  RuleSchema = LA.Type.Object({
    paths: LA.Type.Array(LA.Type.String()),
    ifNoneMatch: LA.Type.Optional(LA.Type.Array(LA.Type.String())),
    ifAnyMatch: LA.Type.Optional(LA.Type.Array(LA.Type.String())),
    source: SourceSchema
  }),
  RulesSchema = LA.Type.Array(RuleSchema),
  RepoRuleSchema = LA.Type.Object({
    rules: RulesSchema,
    last_updated_at: LA.Type.String()
  }),
  ContentRestrictionsResponseSchema = LA.Type.Array(RepoRuleSchema);