var import_os = require("os"),
  path = Dr(require("path"));,function redactPaths(input) {
...
},__name(escapeForRegExp, "escapeForRegExp");,var homedirRegExp = new RegExp("(?<=^|[\\s|(\"'`]|file://)" + escapeForRegExp((0, bW.homedir)()) + "(?=$|[\\\\/:\"'`])", "gi");,function redactHomeDir(input) {
  return input.replace(homedirRegExp, "~");
},__name(redactHomeDir, "redactHomeDir");,var relativePathSuffix = "[\\\\/]?([^:)]*)(?=:\\d)",
  pathSepRegExp = new RegExp(escapeForRegExp(hW.sep), "g"),
...
${redactHomeDir(frame)}` : error.stack += `
  ...
  propertiesSchema = t0.Type.Object({}, {
    additionalProperties: t0.Type.String()
  }),
  measurementsSchema = t0.Type.Object({
    meanLogProb: t0.Type.Optional(t0.Type.Number()),
    meanAlternativeLogProb: t0.Type.Optional(t0.Type.Number())
  }, {
    additionalProperties: t0.Type.Number()
  }),
