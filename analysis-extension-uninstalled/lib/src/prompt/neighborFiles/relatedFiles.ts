var _LRURelatedFileCacheMap = class _LRURelatedFileCacheMap extends LRUCacheMap {
  ...
  return (0, MS.normalizeLanguageId)(languageId) === (0, MS.normalizeLanguageId)(neighborLanguageId);
  ...
  static async getNeighborFiles(ctx, uri, featuresFilterArgs, telemetryData) {
    ...
    if (wksFolder && folder) {
      let docInfo = {
          relativePath: (0, US.relative)(folder, uri.fsPath),
      ...
      if (relatedFiles != null && relatedFiles.size) {
        ...
        relatedFiles.forEach((value, key) => {
          let uri = IL.URI.file(key).toString(),
            relatedFileDocInfo = {
              relativePath: (0, US.relative)(folder, key),
              uri: uri,
              languageId: doc.languageId,
              source: value
            };
