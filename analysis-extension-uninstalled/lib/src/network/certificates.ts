var tls = bn(require("tls"));,var _RootCertificateConfigurator = class _RootCertificateConfigurator {
  constructor(ctx) {
  ...
  async applyToRequestOptions(requestOptions) {
    ...
    requestOptions.secureContext = KM.createSecureContext(options), requestOptions.ca = certs, requestOptions.cert = certs, certs.map(cert => {
