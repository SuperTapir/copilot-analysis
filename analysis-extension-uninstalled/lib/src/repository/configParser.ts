var import_node_os = require("os"),
  import_vscode_uri = bn(Q5());,var logger = new Logger(1, "repository"),
  esc = "\\\\",
  comment = "(?:[#;].*)",
  stringChar = `(?:[^"${esc}]|${esc}.)`,
  keyChar = "[0-9A-Za-z-]",
  ...
  _GitParsingConfigLoader = class _GitParsingConfigLoader extends GitConfigLoader {
    ...
    async baseConfig(ctx, baseConfigFile) {
      let commonUri = await this.commondirConfigUri(ctx, baseConfigFile),
        xdgUri = k1.Utils.joinPath(this.xdgConfigUri(), "git", "config"),
        userUri = k1.Utils.joinPath(this.homeUri(), ".gitconfig");
    ...
    async commondirConfigUri(ctx, baseConfigFile) {
      if (k1.Utils.basename(baseConfigFile).toLowerCase() !== "config.worktree") return;
      let dir = k1.Utils.dirname(baseConfigFile),
        commondirFile = k1.Utils.joinPath(dir, "commondir");
      try {
        let commondirPath = (await ctx.get(FileSystem).readFileString(commondirFile)).trimEnd();
        return k1.Utils.joinPath(resolveFilePath(dir, commondirPath), "config");
      } catch {
        return;
      }
    }
    xdgConfigUri() {
      return process.env.XDG_CONFIG_HOME ? k1.URI.file(process.env.XDG_CONFIG_HOME) : k1.Utils.joinPath(this.homeUri(), ".config");
    }
    homeUri() {
      return k1.URI.file((0, LM.homedir)());
    }
