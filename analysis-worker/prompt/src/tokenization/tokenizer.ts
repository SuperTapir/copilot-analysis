function bytes_to_unicode(map) {
  ...
  init_tokenizer = __esmMin(() => {
    ...
    fs = $t(require("fs")), path = $t(require("path")), import_util = require("util"), range = __name((x, y) => Array.from(Array(y).keys()).slice(x), "range"), ord = __name(x => x.charCodeAt(0), "ord"), chr = __name(x => String.fromCharCode(x), "chr"), textDecoder = new mt.TextDecoder("utf-8"), decodeStr = __name(arr => textDecoder.decode(new Uint8Array(arr)), "decodeStr"), dictZip = __name((x, y) => {
      let result = new Map();
    ...
    TokenizerName = (n => (TokenizerName.cl100k = "cl100k", TokenizerName.mock = "mock", TokenizerName))(pt || {}), tokenizers = new Map();
    ...
    BPETokenizer = class {
      constructor(name = "cl100k") {
        ...
        this.cache = new Map();
        this.textEncoder = new mt.TextEncoder();
        ...
        try {
          let encoder_text = bn.readFileSync(yn.resolve(__dirname, "resources", name, ENCODER));
          encoder_json = JSON.parse(encoder_text.toString()), bpe_file = bn.readFileSync(yn.resolve(__dirname, "resources", name, VOCAB), "utf-8");
        } catch (e) {
